 ...
 * @param addr Output buffer for parsed address (must be at least sizeof(struct in6_addr))
 * @param family Output: AF_INET or AF_INET6
 * @param addr_size Output: size of the address (4 for IPv4, 16 for IPv6)
...
network_add_routing_rule(uint32_t table_id, int family, uint32_t priority)
{
	...
	nl_msg_set_type(req, RTM_NEWRULE);
	nl_msg_set_flags(req, NLM_F_REQUEST | NLM_F_ACK | NLM_F_CREATE | NLM_F_EXCL);
	nl_msg_set_rule_req(req, &rule);
...
network_remove_routing_rule(uint32_t table_id, int family, uint32_t priority)
{
	...
	nl_msg_set_type(req, RTM_DELRULE);
			   ...
			   const char *gateway, const char *dev)
{
	...
	nl_msg_set_type(req, RTM_NEWROUTE);
	nl_msg_set_flags(req, NLM_F_REQUEST | NLM_F_ACK | NLM_F_CREATE | NLM_F_REPLACE);
	nl_msg_set_rt_req(req, &rtmsg);
				...
				const char *gateway, const char *dev)
{
	...
	DEBUG("Removing route %s/%u via %s dev %s from table %u", dest_network, prefix_len,
	      gateway, dev, table_id);
	...
	nl_msg_set_type(req, RTM_DELROUTE);
	nl_msg_set_flags(req, NLM_F_REQUEST | NLM_F_ACK);
	nl_msg_set_rt_req(req, &rtmsg);
